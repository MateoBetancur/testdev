...
console.log('---- EXAMPLE 7 part 1 --- ', findCompany(companies, 1));
console.log('---- EXAMPLE 7 part 2 --- ', filterCompany(companies, 1));
console.log('---- EXAMPLE 7 part 3 --- ', patch(companies, 2));
...
function findCompany(companies, id) {
  ...
  return company ? company.name : 'Don´t exist';
}

/* Part 2 */
function filterCompany(companies, id) {
  const company = companies.filter((data) => data.id != id);
  return company;
}

function patch(companies, id) {
  const company = companies.find((data) => data.id == id);
  const data = {
    name: '/' + company.name,
    isOpen: `/${company.name}/isOpen`,
    userLength: `/${company.name}/userLength`,
  };
  return company ? data : 'Don´t exist';
}
